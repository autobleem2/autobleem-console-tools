...
public:
    ...
    // SCAN, then wait up to timeoutMs for the scan's end (CTRL-EVENT-SCAN-RESULTS on a second, attached
    // connection); true when the results are fresh. A busy scanner (FAIL-BUSY: one is running) is waited for
    // the same way. Either way SCAN_RESULTS answers with what wpa_supplicant has.
    ...
    // password), ENABLE_NETWORK, SAVE_CONFIG. False, with lastError(), at the first step refused.
...
private:
    bool expectOk(const std::string &cmd, const std::string &logged);
