...
struct wpa_ctrl {
	...
	int timeout_ms; /* AutoBleem: the reply timeout, 0 = hostap's 10 s */
#endif /* CONFIG_CTRL_IFACE_UNIX */
...
/* AutoBleem: how long wpa_ctrl_request() waits for a reply, 0 = 10 s */
void wpa_ctrl_set_timeout(struct wpa_ctrl *ctrl, int timeout_ms)
{
	if (ctrl)
...
retry_send:
	...
	for (;;) {
		...
		tv.tv_usec = 0;
#ifdef CONFIG_CTRL_IFACE_UNIX
		/* AutoBleem: a timeout per connection (wpa_ctrl_set_timeout) */
		if (ctrl->timeout_ms > 0) {
			tv.tv_sec = ctrl->timeout_ms / 1000;
			tv.tv_usec = (ctrl->timeout_ms % 1000) * 1000;
		}
#endif /* CONFIG_CTRL_IFACE_UNIX */
		FD_ZERO(&rfds);
		FD_SET(ctrl->s, &rfds);
